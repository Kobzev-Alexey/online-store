  ...
  if (response.ok) {
    ...
    async function init() {
      ...
      for (i = 0; i < dataFromBack.length; i++) {
        const arr1 = dataFromBack[i].courier_location.split(",");
      ...
      for await (object of dataFromBack) {
        ...
        const multiRoute = new ymaps.multiRouter.MultiRoute({
          referencePoints: [arr, object.courier_location],
        });
        const thenum = await multiRoute.model.events.add(
          "requestsuccess",
          async (length) => {
            const distance = await multiRoute
              .getActiveRoute()
